/*
// ngx_mail_mruby_core.c - ngx_mruby mruby module
//
...
static mrb_value ngx_mail_mrb_errlogger(mrb_state *mrb, mrb_value self)
{
  mrb_value msg;
  mrb_int log_level;
  ngx_mail_mruby_internal_ctx_t *ictx = mrb->ud;
  ngx_mail_session_t *s = ictx->s;

  if (s == NULL)
    mrb_raise(mrb, E_RUNTIME_ERROR, "can't use logger at this phase. only use at session mail phase");
...
static mrb_value ngx_mail_mrb_get_ngx_mruby_name(mrb_state *mrb, mrb_value self)
{
  return mrb_str_new_lit(mrb, MODULE_NAME);
}

void ngx_mail_mrb_core_class_init(mrb_state *mrb, struct RClass *class)
{
  ...
  mrb_define_class_method(mrb, class, "errlogger", ngx_mail_mrb_errlogger, MRB_ARGS_REQ(2));
  mrb_define_class_method(mrb, class, "log", ngx_mail_mrb_errlogger, MRB_ARGS_REQ(2));
  mrb_define_class_method(mrb, class, "module_name", ngx_mail_mrb_get_ngx_mruby_name, MRB_ARGS_NONE());
}
